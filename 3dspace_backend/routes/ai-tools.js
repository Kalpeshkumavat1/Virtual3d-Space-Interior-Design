...
const { generateDesignFromPrompt } = require('../services/promptDesignService');
...
// @route   POST /api/ai-tools/prompt-design
// @desc    Convert natural language prompt into structured design plan using AI
// @access  Private (Pro+)
router.post('/prompt-design', [
  authenticateToken,
  requireSubscription('pro'),
  body('prompt')
    .isString()
    ...
    .withMessage('Prompt must be between 20 and 2000 characters'),
  body('constraints')
    .optional()
    .isObject()
    .withMessage('Constraints must be an object'),
  body('constraints.roomType')
    .optional()
    .isString()
    .isLength({ max: 30 }),
  body('constraints.style')
    .optional()
    .isString()
    .isLength({ max: 30 }),
  body('constraints.budget')
    ...
    .isFloat({ min: 0 })
    .withMessage('Budget must be a positive number'),
  body('constraints.currency')
    .optional()
    .isString()
    .isLength({ max: 5 }),
  body('constraints.dimensions')
    .optional()
    .isObject()
    .withMessage('Dimensions must be an object'),
  body('constraints.dimensions.width')
    .optional({ nullable: true })
    .isFloat({ min: 1 })
    .withMessage('Width must be at least 1'),
  body('constraints.dimensions.depth')
    .optional({ nullable: true })
    .isFloat({ min: 1 })
    .withMessage('Depth must be at least 1'),
  body('constraints.dimensions.height')
    .optional({ nullable: true })
    .isFloat({ min: 1 })
    .withMessage('Height must be at least 1'),
  body('constraints.occupants')
    .optional()
    .isInt({ min: 1, max: 20 })
    .withMessage('Occupants must be between 1 and 20'),
  body('constraints.mustHave')
    .optional()
    .isArray({ max: 10 })
    .withMessage('Must-have items must be an array with up to 10 entries'),
  body('constraints.mustHave.*')
    .optional()
    .isString()
    .isLength({ max: 60 }),
  body('constraints.feel')
    .optional()
    .isString()
    .isLength({ max: 40 })
], async (req, res) => {
  try {
    ...
    const aiPlan = await generateDesignFromPrompt({
      prompt,
      constraints,
      userContext: {
    ...
    res.json({
      success: true,
      message: aiPlan.usingFallback
        ? 'AI design generated using fallback data because the AI service is unavailable'
        : 'AI design generated successfully',
      data: aiPlan
    });
  } catch (error) {
    console.error('Prompt design error:', error);
    res.status(500).json({
      success: false,
      message: 'Server error while generating design from prompt'
    });
  }
});
